defmodule RestAPI.Router.Scope do
  alias RestAPI.Router.Scope
  @moduledoc false

  @stack :restapi_router_scopes
  @pipes :restapi_pipeline_scopes
  @top :restapi_top_scopes
  ...
  def route(line, module, kind, verb, path, plug, plug_opts, opts) do
    ...
    as = Keyword.get_lazy(opts, :as, fn -> resource_name(plug, "module") end)
  ...
  defp register_forwards(module, path, plug) when is_atom(plug) do
    ...
    restapi_forwards = Module.get_attribute(module, :restapi_forwards)

    path_segments =
      case Plug.Router.Utils.build_path_match(path) do
        {[], path_segments} ->
          if restapi_forwards[plug] do
            raise ArgumentError,
    ...
    restapi_forwards = Map.put(restapi_forwards, plug, path_segments)
    Module.put_attribute(module, :restapi_forwards, restapi_forwards)
    plug
  end
